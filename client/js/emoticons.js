...
;(function($) {
            ...
            return this.each(function() {
                ...
                $(Obj).bind("click", function(e) {
                    ...
                    for( i = 0; i < options.emotions.length; i++) {
                        faceHtml += '<li text=' + options.emotions[i].phrase + ' type=' + i + '><img title=' + options.emotions[i].phrase + ' src="css/images/emotions/'+ options.emotions[i].url + '"  style="cursor:pointer; position:relative;"   /></li>';
                    }
                    faceHtml += '</ul></div>';
    ...
    //表情文字符号转换为html格式
    String.prototype.emotionsToHtml = function() {
        var rContent = this;
        ...
        var _url = "((https|http|ftp|rtsp|mms)?://)" //
                 ...
                 + "(/[0-9a-z_!~*'().;?:@&=+$,%#-]+)+/?)$";
        var rs = rContent.match(_regx);
        _url = new RegExp(_url,'i');
        var us = rContent.match(_url);
        if(rs) {
            for( i = 0; i < rs.length; i++) {
                for( n = 0; n < em.length; n++) {
                    if(em[n].phrase == rs[i]) {
                        var t = "<img src='css/images/emotions/" + em[n].url + "' />";
        ...
        if(us) {
            for( i = 0; i < us.length; i++) {
                var u = '<a href=' + us[i] + '  target="_blank">'+ us[i] + '</a>';
                rContent = rContent.replace(us[i], u);
                break;
            }
        }
        return rContent;
